...
class RemoteWeatherForcastLoaderTests: XCTestCase {
    ...
    private func  makeSUT(url: URL = URL(string: "any-url.com")!, file: StaticString = #filePath, line: UInt = #line) -> (RemoteCurrentDayWeatherForcastLoader, HTTPClientSpy) {
        let client = HTTPClientSpy()
    ...
    private func makeItem(id: Int?, weather: [CurrentDayWeather]?, main: Main?) -> (model: CurrentDayWeatherForcast, json: [String: Any]) {
        let item = CurrentDayWeatherForcast(id: id, weather: weather, main: main)
    ...
    func expect(_ sut: RemoteCurrentDayWeatherForcastLoader, toCompleteWithResult expectedResult: RemoteCurrentDayWeatherForcastLoader.Result, when action: () -> Void, file: StaticString = #filePath, line: UInt = #line) {
        ...
        sut.load { receivedResult in
            ...
            case let (.failure(receivedError as RemoteCurrentDayWeatherForcastLoader.Error), (.failure(expectedError as RemoteCurrentDayWeatherForcastLoader.Error))):
